...
(function () {
  ...
  /* ---- the second set of captions (2026-09-22) --------------------------
   * The 3 Minute Bible videos carry our reviewed captions burned into the
   * picture. Vimeo ALSO auto-generated a subtitle track on each one and
   * flagged it default:true, so any player that honours that default draws a
   * second, lower-quality set of captions on top of ours. A URL parameter
   * cannot clear it - texttrack=false and texttrack=0 both leave default:true
   * in the player config (verified 2026-09-22). So ask the player to switch it
   * off over the postMessage protocol the embed already speaks: the same
   * effect as the Vimeo Player SDK's disableTextTrack(), without the SDK.
   * Re-sent on play and on texttrackchange because iOS hands fullscreen to the
   * native player, which re-reads the default. Capped so it cannot loop.
   * NOTE: this is a belt to the Vimeo braces - the real fix is clearing
   * 'active' on the autogenerated track at the Vimeo end. */
  function silenceVimeoCaptions(frame) {
    if (!frame || !window.postMessage) return;
    var ORIGIN = "https://player.vimeo.com", tries = 0;
    ...
    window.addEventListener("message", function (e) {
      ...
      var d = e.data;
      try { d = typeof d === "string" ? JSON.parse(d) : d; } catch (x) { return; }
      if (!d || !d.event) return;
      if (d.event === "ready") { tell("addEventListener", "play"); tell("addEventListener", "texttrackchange"); }
      if (tries++ < 8) tell("disableTextTrack");
    });
  }
  ...
  function mqLesson(l) {
    ...
    return '<div class="mq mq-read">' +
      '<a class="mq-bk" href="#"><span class="mq-bkc">&#8249;</span>Back to Lessons</a>' +
      '<div class="mq-ln">Lesson ' + l.n + '</div>' +
      ...
      '</nav>' +
      '<div class="mq-fb"><a class="mq-bk mq-bk-solid" href="#"><span class="mq-bkc">&#8249;</span>Back to Lessons</a></div>' +
      '</div>';
  }
  ...
  function openVideo(n) {
    ...
    silenceVimeoCaptions(vmframe.querySelector("iframe"));
