...
SAME layout/format as Beyond Bumper Stickers (tools/make_pdf.py, v2 clean/print-friendly):
no graph-paper background, no section icons, no per-lesson header art; the packet fonts
(Thierry / Hello-Handmade / Mulish), a thin page border like the art, outlined section
boxes, and display-font question numbers. Only the PALETTE changes, echoing this packet's
own art the way the BBS PDF echoes its own: mustard GOLD #B8860B where BBS uses red,
brown #6D4F26 where BBS uses smoky, navy body ink (the flipbook's "body stays navy for
readability" rule). Supersedes the older tools/make_women_pdf.js.

Re-cut:
  1. pip install reportlab qrcode pillow fonttools brotli cu2qu
  2. content.json = window.BBS_CONTENT dumped from packets/gospel-according-to-the-women/content.js
  3. fonts/ = Thierry.ttf, HelloHandmade.ttf (CFF->glyf via cu2qu),
     Mulish-normal-500/700/800.ttf + Mulish-italic-500.ttf — these MUST be real static
     instances of the google/fonts variable font (fontTools.varLib.instancer, pinning
     wght). If they still carry an `fvar` table every weight silently renders ExtraLight.
  4. cover.png in WORK ; python make_women_pdf.py ; verify every page (PyMuPDF) ; push.
...
GOLD  = HexColor('#B8860B')        # accent  (BBS: red)   — border, numbers, boxes, links
BROWN = HexColor('#6D4F26')        # secondary (BBS: smoky) — eyebrows, refs, footers
NAVY  = HexColor('#0A274C')        # display + body ink
INK   = NAVY
BOX   = GOLD
RULE  = HexColor('#E6D9B8')        # hairline between questions
...
def qr_img(url):
    ...
    img = q.make_image(fill_color='#6D4F26', back_color='white').convert('RGB')
...
def link_box(c, y, title, sub, url, link_label, h=76):
    ...
    if sub:
        c.setFont('Mulish', 9.5); c.setFillColor(BROWN)
        c.drawString(M + 14, y - 38, sub)
    c.setFont('Mulish-Bold', 9); c.setFillColor(GOLD)
    ly = y - h + 15
...
def note_box(c, y, title, sub, note, h=64):
    ...
    if sub:
        c.setFont('Mulish', 9.5); c.setFillColor(BROWN)
        c.drawString(M + 14, y - 38, sub)
    c.setFont('Mulish-It', 9.5); c.setFillColor(BROWN)
    c.drawString(M + 14, y - h + 15, note)
    return y - h

c = canvas.Canvas(os.path.join(WORK, C['meta']['pdf']), pagesize=letter)
c.setTitle('%s — Sunday School Simplified' % C['meta']['title'])
c.setAuthor('The Candler Foundry')
c.setSubject('%s — a six lesson packet' % C['meta']['tagline'])
page = 0
...
L = C['meta']['letter']
border(c)
c.setFillColor(GOLD); c.roundRect(W / 2 - 26, H - 70, 52, 3, 1.5, stroke=0, fill=1)
c.setFont('Hello', 23); c.setFillColor(GOLD)
...
if L.get('quotes'):
    qtop = y - 2
    qs2 = st('Mulish-It', 10.5, BROWN, leading=16)
    for q in L['quotes']:
        y = para(c, esc(q), qs2, M + 16, y - 2, CW - 30)
    c.setStrokeColor(GOLD); c.setLineWidth(2); c.line(M + 4, qtop, M + 4, y + 3)
    y -= 11
for t in L.get('paragraphs2', []) + L.get('paragraphs3', []):
    y = para(c, BOLD.sub(r'<b>\1</b>', esc(t)), body, M, y, CW) - 9
y = para(c, esc(L['grace']), body, M, y - 4, CW)
c.setFont('Hello', 16); c.setFillColor(GOLD)
...
if L.get('rhythmTitle') and L.get('steps'):
    ...
    for i, s in enumerate(steps):
        cx = M + colw * i + colw / 2
        c.setFillColor(GOLD); c.circle(cx, y - 44, 9, stroke=0, fill=1)
...
for l in C['lessons']:
    ...
    # ---- page A ----
    border(c)
    c.setFont('Mulish-XB', 11); c.setFillColor(BROWN)
    c.drawCentredString(W / 2, H - 66, spaced('LESSON ' + NUM[n], 1))
    title = l['title']
    tf = 27 if len(title) <= 26 else (23 if len(title) <= 34 else 20)
    c.setFont('Hello', tf); c.setFillColor(GOLD)
    c.drawCentredString(W / 2, H - 96, title)
    c.setFont('Mulish', 12.5); c.setFillColor(BROWN)
    c.drawCentredString(W / 2, H - 116, l['reference'])
    c.setStrokeColor(GOLD); c.setLineWidth(1.4); c.line(M, H - 126, W - M, H - 126)
    y = H - 150
    ...
    for i, qt in enumerate(qs):
        c.setFont('Thierry', 15); c.setFillColor(GOLD)
        c.drawString(M + 3, y - 12, str(i + 1))
...
# ---------- additional resources (paginates: this packet has videos AND readings) ----------
def res_header(c, first):
    ...
    if first:
        return para(c, 'Optional viewing and reading for classes that want to go deeper.',
                    st('Mulish-It', 10.5, BROWN, leading=15), M, H - 98, CW) - 22
    return H - 104
...
for l in C['lessons']:
    ...
    if o:
        items.append((o['title'], o.get('subtitle') or sub, o.get('url'), 'Watch on Vimeo — or scan the code', sub))
    for r in (l.get('optionalReadings') or []):
        items.append((r['title'], r.get('subtitle') or sub, r.get('url'), 'Open at Bible Gateway — or scan the code', sub))

y = res_header(c, True)
if not items:
    para(c, 'Optional videos and readings will appear here as they are added to future lessons.',
         st('Mulish-It', 10.5, BROWN, leading=15), M, y, CW)
else:
    for title, sub, url, label, lesson_tag in items:
        need = 76 if url else 64
        if y - need < 66:                       # keep clear of the footer
            footer(c, C['meta']['title'], page); c.showPage(); page += 1
            y = res_header(c, False)
        full_sub = '%s · %s' % (lesson_tag, sub) if sub and lesson_tag not in sub else sub
        if url:
            y = link_box(c, y, title, full_sub, url, label) - 16
        else:
...
# ---------- end page ----------
border(c)
c.setFillColor(GOLD); c.roundRect(W / 2 - 26, H / 2 + 96, 52, 3, 1.5, stroke=0, fill=1)
c.setFont('Hello', 30); c.setFillColor(GOLD)
c.drawCentredString(W / 2, H / 2 + 54, 'The Candler Foundry')
# flow the URL from the paragraph's real bottom — at true Mulish weights this blurb
# wraps to 3 lines and collided with a fixed-y URL
ey = para(c, esc('%s is a project of The Candler Foundry, making the best of biblical scholarship accessible to everyone.' % C['meta']['series']),
          st('Mulish-It', 10.5, BROWN, leading=15.5, align=1), W / 2 - 165, H / 2 + 34, 330)
...
c.setFont('Mulish', 8); c.setFillColor(BROWN)
...
c.showPage()
c.save()
print('PDF written:', page + 1, 'pages ->', C['meta']['pdf'])
